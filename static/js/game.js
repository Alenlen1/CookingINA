  ...
  const RECIPES = [
    ...
    { name:'Sinigang',  emoji:'🍜', ingredients:['🥩','🍅','🥬','🧅','🌶️'] },
    { name:'Kare-Kare', emoji:'🥘', ingredients:['🥜','🍆','🥩','🥬','🧅'] },
    { name:'Lechon',    emoji:'🐷', ingredients:['🥩','🧄','🌿','🍋','🧂'] },
    { name:'Tinola',    emoji:'🍗', ingredients:['🍗','🧄','🧅','🌿','🥬'] },
    { name:'Caldereta', emoji:'🫕', ingredients:['🥩','🥕','🥔','🍅','🌶️'] },
  ];

  const QUIZ_QUESTIONS = [
    { q:'Which ingredient makes Sinigang sour?',  opts:['Tamarind','Vinegar','Calamansi','Lemon'],   ans:0 },
    { q:'Adobo is cooked with soy sauce and?',    opts:['Lemon','Vinegar','Ketchup','Fish sauce'],   ans:1 },
    { q:'What nut is Kare-Kare famous for?',      opts:['Cashew','Almond','Peanut','Coconut'],       ans:2 },
    { q:'Tinola is a Filipino?',                  opts:['Dessert','Soup','Salad','Noodle dish'],     ans:1 },
    { q:'Lechon is a whole roasted?',             opts:['Chicken','Beef','Goat','Pork'],             ans:3 },
    { q:'What is Bagoong?',                       opts:['A rice','A shrimp paste','A vegetable','A fruit'], ans:1 },
    { q:'Which dish uses pork and liver sauce?',  opts:['Adobo','Tinola','Kare-Kare','Sinigang'],    ans:2 },
    { q:'What is the base of Arroz Caldo?',       opts:['Noodles','Rice porridge','Bread','Corn'],   ans:1 },
    { q:'Pancit is a Filipino?',                  opts:['Noodle dish','Rice cake','Soup','Salad'],   ans:0 },
    { q:'Which spice gives Caldereta its kick?',  opts:['Ginger','Chili','Turmeric','Cumin'],        ans:1 },
  ];
  ...
  /* ── STATE ── */
  let score=0, lives=3, stageIdx=0, recipe=null, gameActive=false;
  let timerInterval=null, fryInterval=null, catchInterval=null;
  let timeLeft=0, combo=1, highScore=0;
  let stirPrev=null, stirTotal=0, stirRequired=360*2.2;
  let fryPos=0, fryDir=1, fryMin=32, fryMax=60;
  let seasonCount=0, chopDone=0, chopTotal=0;
  let catchScore=0, catchBasketX=0, catchItems=[], catchCtx=null;
  let quizAnswered=false;
  let plateSelected=[], plateCorrect=[], plateWrong=[];
  ...
  function flash(msg, col){
    ...
    if(!fb)return;
    fb.textContent=msg; fb.style.color=col;
  ...
  function updateCombo(){
    ...
    if(!cb)return;
    if(combo>=2){ cb.textContent='🔥 x'+combo+' COMBO!'; cb.classList.add('show'); }
  ...
  function stageComplete(base){
    ...
    flash(msg, '#22c55e');
    setTimeout(()=>{ stageIdx++; showStage(); }, 1100);
  }
  function startTimer(secs){
    ...
    timerInterval=setInterval(()=>{
      ...
      if(bar){ bar.style.width=pct+'%'; bar.style.background=pct>40?'#e67e22':'#ef4444'; }
  ...
  function startGame(count){
    score=0; lives=3; stageIdx=0; combo=1;
  ...
  function showStage(){
    if(stageIdx>=STAGES.length){ showResult(); return; }
  ...
  /* ══ CHOP ══ */
  function initChop(){
    chopDone=0; chopTotal=recipe.ingredients.length;
    const area=document.getElementById('stageArea');
    const items=recipe.ingredients.map((e,i)=>
  ...
  function initStir(){
    ...
    function onMove(cx,cy){
      ...
      const x=cx-r.left-90, y=cy-r.top-90;
    ...
    cv.addEventListener('mouseleave',()=>{ stirPrev=null; });
  }
  function drawPot(cv,pct){
    ...
    if(pct>0){
      ...
      ctx.strokeStyle='rgba(34,197,94,0.7)'; ctx.lineWidth=9; ctx.stroke();
    }
  }

  /* ══ FRY ══ */
  function initFry(){
    fryPos=0; fryDir=1; fryMin=32; fryMax=60;
    const area=document.getElementById('stageArea');
    area.innerHTML+=`
    ...
    if(sw){ sw.style.left=fryMin+'%'; sw.style.width=(fryMax-fryMin)+'%'; }
    fryInterval=setInterval(()=>{
      fryPos+=fryDir*2.2;
      if(fryPos>=100){fryPos=100;fryDir=-1;}
      if(fryPos<=0){fryPos=0;fryDir=1;}
  ...
  window.stopFry=function(){
    if(!gameActive)return;
    clearInterval(fryInterval);
    if(fryPos>=fryMin&&fryPos<=fryMax){ stageComplete(160); }
    else {
  ...
  window.shake=function(i){
    ...
    if(bowl){bowl.style.transform='scale(1.18)';setTimeout(()=>bowl.style.transform='',140);}
  ...
  function initCatch(){
    ...
    catchInterval=setInterval(()=>{
      if(!gameActive){clearInterval(catchInterval);return;}
      if(Math.random()<0.18){
        catchItems.push({x:rand(20,280),y:-20,speed:rand(2,5),emoji:foods[rand(0,foods.length)]});
      }
    ...
    cv.addEventListener('touchmove',e=>{e.preventDefault();moveCatch(e.touches[0].clientX,e.touches[0].clientY);},{passive:false});
  }
  ...
  window.answerQuiz=function(el,chosen,correct){
    ...
    if(chosen===correct){
      ...
      flash('❌ Wrong!','#ef4444');
      setTimeout(()=>{
        if(lives<=1){lives--;showResult();}
        else{lives--;stageIdx++;showStage();}
      },1200);
    }
  };
  ...
  window.checkPlate=function(){
    ...
    if(chosen===correct){ stageComplete(140); }
    else {
  ...
  function initMemory(){
    ...
    memCards=cards.map((e,i)=>({emoji:e,id:i,flipped:false,matched:false}));
    const area=document.getElementById('stageArea');
    area.innerHTML+=`
  ...
  window.flipCard=function(i){
    ...
    if(el){ el.textContent=c.emoji; el.classList.add('flip'); }
    memFlipped.push(i);
      ...
      if(memCards[a].emoji===memCards[b].emoji){
        ...
        setTimeout(()=>{
          [a,b].forEach(idx2=>{
            ...
            if(e){ e.textContent='❓'; e.classList.remove('flip'); }
          });
          memFlipped=[]; memLock=false;
        },900);
      }
  ...
  function showResult(){
    ...
    let grade,emoji,msg;
    if(score>=700){grade='S';emoji='🏆';msg='Perfect! Mama is crying happy tears!';}
    else if(score>=550){grade='A';emoji='⭐';msg='Excellent! Mama would be proud!';}
    else if(score>=380){grade='B';emoji='😊';msg='Good job! Keep practicing!';}
    else if(score>=200){grade='C';emoji='😅';msg='Not bad, but Mama cried a little.';}
    else{grade='D';emoji='😢';msg='Mama will redo this for you...';}
