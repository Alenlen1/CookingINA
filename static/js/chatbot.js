...
function scrollToBottom(smooth = true) {
  const el = document.getElementById('chatMessages') || document.getElementById('widgetMessages');
  if (!el) return;
...
function _speakText(text) {
  ...
  const speak = () => {
    ...
    utter.onerror = (e) => { console.warn('Speech error:', e); isSpeaking = false; };
...
async function sendChatMessage() {
  ...
  try {
    ...
    if (typing) typing.style.display = 'none';
    _appendMessage('ai', '❌ Network error. Please check your connection and try again.');
  }
...
function _appendMessage(role, content) {
  ...
  if (isUser) {
    ...
    avatarHtml = `<div class="msg-avatar ai-avatar">🍳</div>`;
  }
...
async function sendWidgetMessage() {
  ...
  try {
    ...
    if (data.reply) {
      ...
      _appendWidgetMessage('ai', '❌ ' + (data.error || 'Something went wrong.'));
    }
  } catch (e) {
    if (typing) typing.style.display = 'none';
...
function _appendWidgetMessage(role, content) {
  ...
  const avatarHtml = `<div class="widget-avatar">${isUser ? '👤' : '🍳'}</div>`;
