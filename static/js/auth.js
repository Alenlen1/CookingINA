...
function getOtpValue(prefix = '') {
  const container = document.getElementById(prefix ? `${prefix}OtpInputs` : 'otpInputs');
  if (!container) return '';
...
async function initiateRegister() {
  ...
  if (!username || !email || !password || !confirm) {
    showBanner('regError', 'Please fill in all fields.'); return;
  }
  if (password !== confirm) {
    showBanner('regError', 'Passwords do not match.'); return;
  }
  if (password.length < 8) {
    showBanner('regError', 'Password must be at least 8 characters.'); return;
  }
...
async function verifyRegCode() {
  ...
  if (code.length < 6) {
    showBanner('otpError', 'Please enter all 6 digits.'); return;
  }
  ...
  try {
    const { ok, data } = await apiPost('/register/verify',
      { email: _regEmail, code });

    if (data.ok) {
      showBanner('otpSuccess', data.message || '🎉 Account created!', 'success');
      clearBanner('otpError');
      setTimeout(() => {
...
async function resendRegCode() {
  ...
  try {
    const { ok, data } = await apiPost('/register/resend', { email: _regEmail });
    if (data.ok) {
      showBanner('otpSuccess', data.message, 'success');
      startCountdown('otpResendBtn', 'otpCountdown', 30);
    } else {
      showBanner('otpError', data.error);
      if (data.cooldown) startCountdown('otpResendBtn', 'otpCountdown', data.cooldown);
      if (data.restart) {
        closeOtpModal();
        showBanner('regError', data.error);
      }
...
async function fpSendCode() {
  ...
  if (!email) { showBanner('fp1Error', 'Please enter your email address.'); return; }
...
async function fpVerifyCode() {
  ...
  if (code.length < 6) { showBanner('fp2Error', 'Please enter all 6 digits.'); return; }

  setLoading('fpVerifyBtn', 'fpVerifyText', 'fpVerifySpinner', true);

  try {
    const { ok, data } = await apiPost('/forgot-password/verify', { email: _fpEmail, code });
    ...
    if (data.ok) {
      ...
      if (data.restart) { setTimeout(() => fpGoStep(1), 1500); }
      if (data.expired) {
        clearOtpInputs('fp');
...
async function fpResetPassword() {
  ...
  if (!password || !confirm) { showBanner('fp3Error', 'Please fill in both fields.'); return; }
  if (password.length < 8)   { showBanner('fp3Error', 'Password must be at least 8 characters.'); return; }
  if (password !== confirm)  { showBanner('fp3Error', 'Passwords do not match.'); return; }

  setLoading('fpResetBtn', 'fpResetText', 'fpResetSpinner', true);

  try {
    const { ok, data } = await apiPost('/forgot-password/reset',
      { email: _fpEmail, password, confirm });

    if (data.ok) {
      showBanner('fp3Success', data.message || '✅ Password updated!', 'success');
...
async function fpResendCode() {
  ...
  try {
    const { ok, data } = await apiPost('/forgot-password/resend', { email: _fpEmail });
    if (data.ok) {
      showBanner('fp2Success', data.message, 'success');
      startCountdown('fpResendBtn', 'fpCountdown', 30);
    } else {
      showBanner('fp2Error', data.error);
      if (data.cooldown) startCountdown('fpResendBtn', 'fpCountdown', data.cooldown);
      if (data.restart) { fpGoStep(1); showBanner('fp1Error', data.error); }
    }
  } catch (e) {
    showBanner('fp2Error', 'Network error. Please try again.');
  }
...
async function changePassword() {
  ...
  if (!current_password || !new_password || !confirm_password) {
    showBanner('cpError', 'Please fill in all password fields.'); return;
  }
  if (new_password.length < 8) {
    showBanner('cpError', 'New password must be at least 8 characters.'); return;
  }
  if (new_password !== confirm_password) {
    showBanner('cpError', 'New passwords do not match.'); return;
  }
  ...
  try {
    ...
    if (data.ok) {
      showBanner('cpSuccess', data.message || '✅ Password changed!', 'success');
